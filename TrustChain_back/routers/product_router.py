...
from models.Product_model import Product, ProductInDB
...
def update_product(product_id: str, product: ProductUpdate, db: Database = Depends(get_db)):
    # Validate the product_id and ensure the product exists in the database
    product_id = str_to_objectid(product_id)
